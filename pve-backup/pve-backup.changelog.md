...
## v1.8.1
- Added quick remote storage usability test before expensive remote operations.
- Added `STORAGE_USABLE_TIMEOUT=8` for fast detection of mounts that are reachable but not usable.
- Added `check_storage_usable()` using bounded `test`, `stat`, and `ls`.
- NFS/CIFS storages can now be skipped earlier when they respond to ping but behave like stale/zombie mounts.
- Improved skip reason reporting with `unusable-or-unreachable`.
- Preserved final summary and `--restore-plan`.
